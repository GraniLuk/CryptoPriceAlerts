...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        filtered_alerts = [
            alert for alert in current_alerts if alert.get("id") != alert_id
        ]

        if len(filtered_alerts) == len(current_alerts):
            return func.HttpResponse(
                f"Alert with ID {alert_id} not found", status_code=404
            )

        save_alerts_to_azure("alerts.json", filtered_alerts)

        return func.HttpResponse("Alert removed successfully", status_code=200)

    except Exception as e:
        return func.HttpResponse(f"Error removing alert: {str(e)}", status_code=500)
